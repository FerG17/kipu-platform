...
public class AuthenticationHardeningTests(BodegaApiFactory factory) : IntegrationTestBase(factory)
{
    /// <summary>Mirrors BodegaApiFactory's test signing key — the one the app really uses in this suite.</summary>
    ...
    public async Task ErrorResponses_DoNotLeakInternals()
    {
        ...
        foreach (var leak in new[] { "StackTrace", "at Bodega.Platform", "MySql", "SELECT ", "Microsoft.EntityFrameworkCore" })
