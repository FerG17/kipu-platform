...
public class SaleQueryService(ISaleRepository saleRepository, IPaymentPlanRepository paymentPlanRepository) : ISaleQueryService
{
    ...
    public async Task<PagedResult<Sale>> Handle(GetSalesPageByBusinessIdQuery query, CancellationToken cancellationToken)
    {
        return await saleRepository.FindPageByBusinessIdAsync(query.BusinessId, query.DateFrom, query.DateTo, query.Page,
            cancellationToken);
    }
