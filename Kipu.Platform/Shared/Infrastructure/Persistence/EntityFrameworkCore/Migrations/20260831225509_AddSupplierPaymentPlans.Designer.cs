// <auto-generated />
using System;
using Kipu.Platform.Shared.Infrastructure.Persistence.EntityFrameworkCore.Configuration;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Kipu.Platform.Shared.Infrastructure.Persistence.EntityFrameworkCore.Migrations
{
    [DbContext(typeof(AppDbContext))]
    [Migration("20260831225509_AddSupplierPaymentPlans")]
    partial class AddSupplierPaymentPlans
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "10.0.10")
                .HasAnnotation("Relational:MaxIdentifierLength", 64);

            modelBuilder.Entity("Kipu.Platform.Alerts.Domain.Model.Aggregates.Alert", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<decimal?>("Amount")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("amount");

                    b.Property<int?>("BatchId")
                        .HasColumnType("int")
                        .HasColumnName("batch_id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<decimal?>("CurrentStock")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("current_stock");

                    b.Property<string>("CustomerOrSupplierName")
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("customer_or_supplier_name");

                    b.Property<DateTimeOffset>("Date")
                        .HasColumnType("datetime")
                        .HasColumnName("date");

                    b.Property<int?>("DaysRemaining")
                        .HasColumnType("int")
                        .HasColumnName("days_remaining");

                    b.Property<int?>("DaysToExpiry")
                        .HasColumnType("int")
                        .HasColumnName("days_to_expiry");

                    b.Property<string>("Message")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)")
                        .HasColumnName("message");

                    b.Property<decimal?>("MinStock")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("min_stock");

                    b.Property<bool>("Notified")
                        .HasColumnType("tinyint(1)")
                        .HasColumnName("notified");

                    b.Property<DateTimeOffset?>("NotifiedAt")
                        .HasColumnType("datetime")
                        .HasColumnName("notified_at");

                    b.Property<int?>("ProductId")
                        .HasColumnType("int")
                        .HasColumnName("product_id");

                    b.Property<string>("ProductName")
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("product_name");

                    b.Property<int?>("PurchaseOrderId")
                        .HasColumnType("int")
                        .HasColumnName("purchase_order_id");

                    b.Property<DateTimeOffset?>("ResolvedAt")
                        .HasColumnType("datetime")
                        .HasColumnName("resolved_at");

                    b.Property<int?>("SaleId")
                        .HasColumnType("int")
                        .HasColumnName("sale_id");

                    b.Property<string>("Severity")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("severity");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("status");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("varchar(30)")
                        .HasColumnName("type");

                    b.Property<int?>("WarehouseId")
                        .HasColumnType("int")
                        .HasColumnName("warehouse_id");

                    b.HasKey("Id")
                        .HasName("pk_alerts");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_alerts_business_id");

                    b.HasIndex("ProductId")
                        .HasDatabaseName("ix_alerts_product_id");

                    b.HasIndex("WarehouseId")
                        .HasDatabaseName("ix_alerts_warehouse_id");

                    b.HasIndex("BatchId", "Type", "Status")
                        .HasDatabaseName("ix_alerts_batch_id_type_status");

                    b.HasIndex("PurchaseOrderId", "Type", "Status")
                        .HasDatabaseName("ix_alerts_purchase_order_id_type_status");

                    b.HasIndex("SaleId", "Type", "Status")
                        .HasDatabaseName("ix_alerts_sale_id_type_status");

                    b.ToTable("alerts");
                });

            modelBuilder.Entity("Kipu.Platform.Alerts.Domain.Model.Entities.AlertRule", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<string>("AlertType")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("varchar(30)")
                        .HasColumnName("alert_type");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<bool>("Enabled")
                        .HasColumnType("tinyint(1)")
                        .HasColumnName("enabled");

                    b.Property<int>("ThresholdValue")
                        .HasColumnType("int")
                        .HasColumnName("threshold_value");

                    b.HasKey("Id")
                        .HasName("pk_alert_rules");

                    b.HasIndex("BusinessId", "AlertType")
                        .IsUnique()
                        .HasDatabaseName("ix_alert_rules_business_id_alert_type");

                    b.ToTable("alert_rules");
                });

            modelBuilder.Entity("Kipu.Platform.Dashboard.Domain.Model.Entities.Report", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<DateTime?>("DateFrom")
                        .HasColumnType("date")
                        .HasColumnName("date_from");

                    b.Property<DateTime?>("DateTo")
                        .HasColumnType("date")
                        .HasColumnName("date_to");

                    b.Property<DateTimeOffset>("GeneratedAt")
                        .HasColumnType("datetime")
                        .HasColumnName("generated_at");

                    b.Property<int?>("ProductId")
                        .HasColumnType("int")
                        .HasColumnName("product_id");

                    b.Property<int?>("SupplierId")
                        .HasColumnType("int")
                        .HasColumnName("supplier_id");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("type");

                    b.HasKey("Id")
                        .HasName("pk_reports");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_reports_business_id");

                    b.ToTable("reports");
                });

            modelBuilder.Entity("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)")
                        .HasColumnName("address");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("name");

                    b.Property<string>("Ruc")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("ruc");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("type");

                    b.Property<int>("UserId")
                        .HasColumnType("int")
                        .HasColumnName("user_id");

                    b.HasKey("Id")
                        .HasName("pk_businesses");

                    b.ToTable("businesses");
                });

            modelBuilder.Entity("Kipu.Platform.Iam.Domain.Model.Aggregates.User", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)")
                        .HasColumnName("email");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)")
                        .HasColumnName("last_name");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("varchar(100)")
                        .HasColumnName("name");

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasColumnType("longtext")
                        .HasColumnName("password_hash");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("varchar(30)")
                        .HasColumnName("phone");

                    b.Property<int>("RoleId")
                        .HasColumnType("int")
                        .HasColumnName("role_id");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("status");

                    b.Property<int>("TokenVersion")
                        .HasColumnType("int")
                        .HasColumnName("token_version");

                    b.Property<int>("Version")
                        .IsConcurrencyToken()
                        .HasColumnType("int")
                        .HasColumnName("version");

                    b.HasKey("Id")
                        .HasName("pk_users");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_users_business_id");

                    b.HasIndex("Email")
                        .IsUnique()
                        .HasDatabaseName("ix_users_email");

                    b.HasIndex("RoleId")
                        .HasDatabaseName("ix_users_role_id");

                    b.ToTable("users");
                });

            modelBuilder.Entity("Kipu.Platform.Iam.Domain.Model.Entities.PasswordResetCode", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("AttemptCount")
                        .HasColumnType("int")
                        .HasColumnName("attempt_count");

                    b.Property<string>("CodeHash")
                        .IsRequired()
                        .HasColumnType("longtext")
                        .HasColumnName("code_hash");

                    b.Property<DateTimeOffset>("ExpiresAt")
                        .HasColumnType("datetime(6)")
                        .HasColumnName("expires_at");

                    b.Property<bool>("IsVerified")
                        .HasColumnType("tinyint(1)")
                        .HasColumnName("is_verified");

                    b.Property<DateTimeOffset>("RequestedAt")
                        .HasColumnType("datetime(6)")
                        .HasColumnName("requested_at");

                    b.Property<int>("UserId")
                        .HasColumnType("int")
                        .HasColumnName("user_id");

                    b.Property<int>("Version")
                        .IsConcurrencyToken()
                        .HasColumnType("int")
                        .HasColumnName("version");

                    b.HasKey("Id")
                        .HasName("pk_password_reset_codes");

                    b.HasIndex("UserId")
                        .HasDatabaseName("ix_password_reset_codes_user_id");

                    b.ToTable("password_reset_codes");
                });

            modelBuilder.Entity("Kipu.Platform.Iam.Domain.Model.Entities.Role", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<string>("Position")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("varchar(30)")
                        .HasColumnName("position");

                    b.HasKey("Id")
                        .HasName("pk_roles");

                    b.ToTable("roles");

                    b.HasData(
                        new
                        {
                            Id = 1,
                            Position = "ADMIN"
                        },
                        new
                        {
                            Id = 2,
                            Position = "CASHIER"
                        },
                        new
                        {
                            Id = 3,
                            Position = "WAREHOUSE"
                        });
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Aggregates.Category", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("name");

                    b.HasKey("Id")
                        .HasName("pk_categories");

                    b.HasIndex("BusinessId", "Name")
                        .IsUnique()
                        .HasDatabaseName("ix_categories_business_id_name");

                    b.ToTable("categories");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Aggregates.Product", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<string>("Barcode")
                        .HasMaxLength(64)
                        .HasColumnType("varchar(64)")
                        .HasColumnName("barcode");

                    b.Property<decimal>("BasePrice")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("base_price");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("category");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)")
                        .HasColumnName("description");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("name");

                    b.Property<string>("Presentacion")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("presentacion");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("status");

                    b.Property<string>("UnidadDeMedida")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("unidad_de_medida");

                    b.Property<string>("UnitOfSale")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("unit_of_sale");

                    b.HasKey("Id")
                        .HasName("pk_products");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_products_business_id");

                    b.ToTable("products");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Aggregates.Warehouse", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)")
                        .HasColumnName("address");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<string>("Capacity")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("capacity");

                    b.Property<string>("Code")
                        .IsRequired()
                        .HasMaxLength(30)
                        .HasColumnType("varchar(30)")
                        .HasColumnName("code");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("name");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("status");

                    b.HasKey("Id")
                        .HasName("pk_warehouses");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_warehouses_business_id");

                    b.ToTable("warehouses");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Entities.Batch", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<DateTime?>("Expiration")
                        .HasColumnType("date")
                        .HasColumnName("expiration");

                    b.Property<int>("InventoryItemId")
                        .HasColumnType("int")
                        .HasColumnName("inventory_item_id");

                    b.Property<string>("Label")
                        .HasMaxLength(60)
                        .HasColumnType("varchar(60)")
                        .HasColumnName("label");

                    b.Property<int>("ProductId")
                        .HasColumnType("int")
                        .HasColumnName("product_id");

                    b.Property<decimal>("PurchasePrice")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("purchase_price");

                    b.Property<decimal>("Quantity")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("quantity");

                    b.Property<decimal>("RemainingQuantity")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("remaining_quantity");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("status");

                    b.HasKey("Id")
                        .HasName("pk_batches");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_batches_business_id");

                    b.HasIndex("InventoryItemId")
                        .HasDatabaseName("ix_batches_inventory_item_id");

                    b.HasIndex("ProductId")
                        .HasDatabaseName("ix_batches_product_id");

                    b.ToTable("batches");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Entities.InventoryItem", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<decimal>("MinimumStock")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("minimum_stock");

                    b.Property<int>("ProductId")
                        .HasColumnType("int")
                        .HasColumnName("product_id");

                    b.Property<decimal>("StockUnit")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("stock_unit");

                    b.Property<DateTimeOffset>("UpdatedAt")
                        .HasColumnType("datetime")
                        .HasColumnName("updated_at");

                    b.Property<int>("Version")
                        .IsConcurrencyToken()
                        .HasColumnType("int")
                        .HasColumnName("version");

                    b.Property<int>("WarehouseId")
                        .HasColumnType("int")
                        .HasColumnName("warehouse_id");

                    b.HasKey("Id")
                        .HasName("pk_inventory_items");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_inventory_items_business_id");

                    b.HasIndex("WarehouseId")
                        .HasDatabaseName("ix_inventory_items_warehouse_id");

                    b.HasIndex("ProductId", "WarehouseId")
                        .IsUnique()
                        .HasDatabaseName("ix_inventory_items_product_id_warehouse_id");

                    b.ToTable("inventory_items");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Entities.ProductSupplier", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<int>("ProductId")
                        .HasColumnType("int")
                        .HasColumnName("product_id");

                    b.Property<int>("SupplierId")
                        .HasColumnType("int")
                        .HasColumnName("supplier_id");

                    b.HasKey("Id")
                        .HasName("pk_product_suppliers");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_product_suppliers_business_id");

                    b.HasIndex("ProductId", "SupplierId")
                        .IsUnique()
                        .HasDatabaseName("ix_product_suppliers_product_id_supplier_id");

                    b.ToTable("product_suppliers");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Entities.StockMovement", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int?>("BatchId")
                        .HasColumnType("int")
                        .HasColumnName("batch_id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<string>("Note")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)")
                        .HasColumnName("note");

                    b.Property<int>("ProductId")
                        .HasColumnType("int")
                        .HasColumnName("product_id");

                    b.Property<decimal>("Quantity")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("quantity");

                    b.Property<DateTimeOffset>("RegisteredAt")
                        .HasColumnType("datetime")
                        .HasColumnName("registered_at");

                    b.Property<string>("Supplier")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("supplier");

                    b.Property<int?>("SupplierId")
                        .HasColumnType("int")
                        .HasColumnName("supplier_id");

                    b.Property<string>("Type")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("type");

                    b.Property<int>("WarehouseId")
                        .HasColumnType("int")
                        .HasColumnName("warehouse_id");

                    b.HasKey("Id")
                        .HasName("pk_stock_movements");

                    b.HasIndex("BatchId")
                        .HasDatabaseName("ix_stock_movements_batch_id");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_stock_movements_business_id");

                    b.HasIndex("ProductId")
                        .HasDatabaseName("ix_stock_movements_product_id");

                    b.HasIndex("WarehouseId")
                        .HasDatabaseName("ix_stock_movements_warehouse_id");

                    b.ToTable("stock_movements");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Aggregates.Customer", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<string>("DocumentNumber")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("document_number");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("email");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("full_name");

                    b.Property<string>("PhoneNumber")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("phone_number");

                    b.Property<DateTimeOffset>("RegisteredAt")
                        .HasColumnType("datetime")
                        .HasColumnName("registered_at");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("status");

                    b.HasKey("Id")
                        .HasName("pk_customers");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_customers_business_id");

                    b.ToTable("customers");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Aggregates.Sale", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<string>("Currency")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("varchar(10)")
                        .HasColumnName("currency");

                    b.Property<int?>("CustomerId")
                        .HasColumnType("int")
                        .HasColumnName("customer_id");

                    b.Property<DateTimeOffset>("Date")
                        .HasColumnType("datetime")
                        .HasColumnName("date");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)")
                        .HasColumnName("description");

                    b.Property<string>("IdempotencyKey")
                        .HasMaxLength(64)
                        .HasColumnType("varchar(64)")
                        .HasColumnName("idempotency_key");

                    b.Property<string>("PaymentMethod")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("payment_method");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("status");

                    b.Property<decimal>("TotalAmount")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("total_amount");

                    b.Property<int>("Version")
                        .IsConcurrencyToken()
                        .HasColumnType("int")
                        .HasColumnName("version");

                    b.HasKey("Id")
                        .HasName("pk_sales");

                    b.HasIndex("CustomerId")
                        .HasDatabaseName("ix_sales_customer_id");

                    b.HasIndex("BusinessId", "IdempotencyKey")
                        .IsUnique()
                        .HasDatabaseName("ix_sales_business_id_idempotency_key");

                    b.ToTable("sales");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Entities.InstallmentPayment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("amount");

                    b.Property<bool>("IsReversed")
                        .HasColumnType("tinyint(1)")
                        .HasColumnName("is_reversed");

                    b.Property<DateTimeOffset>("PaidAt")
                        .HasColumnType("datetime")
                        .HasColumnName("paid_at");

                    b.Property<int>("PaidByUserId")
                        .HasColumnType("int")
                        .HasColumnName("paid_by_user_id");

                    b.Property<int?>("PaymentInstallmentId")
                        .HasColumnType("int")
                        .HasColumnName("payment_installment_id");

                    b.Property<int>("PaymentPlanId")
                        .HasColumnType("int")
                        .HasColumnName("payment_plan_id");

                    b.Property<DateTimeOffset?>("ReversedAt")
                        .HasColumnType("datetime")
                        .HasColumnName("reversed_at");

                    b.Property<int?>("ReversedByUserId")
                        .HasColumnType("int")
                        .HasColumnName("reversed_by_user_id");

                    b.HasKey("Id")
                        .HasName("pk_installment_payments");

                    b.HasIndex("PaymentInstallmentId")
                        .HasDatabaseName("ix_installment_payments_payment_installment_id");

                    b.HasIndex("PaymentPlanId")
                        .HasDatabaseName("ix_installment_payments_payment_plan_id");

                    b.ToTable("installment_payments");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Entities.PaymentInstallment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("amount");

                    b.Property<DateTime>("DueDate")
                        .HasColumnType("date")
                        .HasColumnName("due_date");

                    b.Property<bool>("IsPaid")
                        .HasColumnType("tinyint(1)")
                        .HasColumnName("is_paid");

                    b.Property<int>("Number")
                        .HasColumnType("int")
                        .HasColumnName("number");

                    b.Property<int>("PaymentPlanId")
                        .HasColumnType("int")
                        .HasColumnName("payment_plan_id");

                    b.HasKey("Id")
                        .HasName("pk_payment_installments");

                    b.HasIndex("PaymentPlanId")
                        .HasDatabaseName("ix_payment_installments_payment_plan_id");

                    b.ToTable("payment_installments");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Entities.PaymentPlan", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<bool>("IsCancelled")
                        .HasColumnType("tinyint(1)")
                        .HasColumnName("is_cancelled");

                    b.Property<int>("PaidInstallments")
                        .HasColumnType("int")
                        .HasColumnName("paid_installments");

                    b.Property<int>("SaleId")
                        .HasColumnType("int")
                        .HasColumnName("sale_id");

                    b.Property<int>("TotalInstallments")
                        .HasColumnType("int")
                        .HasColumnName("total_installments");

                    b.Property<int>("Version")
                        .IsConcurrencyToken()
                        .HasColumnType("int")
                        .HasColumnName("version");

                    b.HasKey("Id")
                        .HasName("pk_payment_plans");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_payment_plans_business_id");

                    b.HasIndex("SaleId")
                        .IsUnique()
                        .HasDatabaseName("ix_payment_plans_sale_id");

                    b.ToTable("payment_plans");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Entities.SaleDetail", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<decimal>("Discount")
                        .HasColumnType("decimal(5,4)")
                        .HasColumnName("discount");

                    b.Property<int>("ProductId")
                        .HasColumnType("int")
                        .HasColumnName("product_id");

                    b.Property<decimal>("Quantity")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("quantity");

                    b.Property<int>("SaleId")
                        .HasColumnType("int")
                        .HasColumnName("sale_id");

                    b.Property<decimal>("UnitPrice")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("unit_price");

                    b.HasKey("Id")
                        .HasName("pk_sale_details");

                    b.HasIndex("ProductId")
                        .HasDatabaseName("ix_sale_details_product_id");

                    b.HasIndex("SaleId")
                        .HasDatabaseName("ix_sale_details_sale_id");

                    b.ToTable("sale_details");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Aggregates.PurchaseOrder", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<string>("Currency")
                        .IsRequired()
                        .HasMaxLength(10)
                        .HasColumnType("varchar(10)")
                        .HasColumnName("currency");

                    b.Property<DateTime>("Date")
                        .HasColumnType("date")
                        .HasColumnName("date");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("varchar(500)")
                        .HasColumnName("description");

                    b.Property<DateTime?>("ExpectedDate")
                        .HasColumnType("date")
                        .HasColumnName("expected_date");

                    b.Property<DateTime?>("ReceivedDate")
                        .HasColumnType("date")
                        .HasColumnName("received_date");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("status");

                    b.Property<int>("SupplierId")
                        .HasColumnType("int")
                        .HasColumnName("supplier_id");

                    b.Property<int>("Version")
                        .IsConcurrencyToken()
                        .HasColumnType("int")
                        .HasColumnName("version");

                    b.HasKey("Id")
                        .HasName("pk_purchase_orders");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_purchase_orders_business_id");

                    b.HasIndex("SupplierId")
                        .HasDatabaseName("ix_purchase_orders_supplier_id");

                    b.ToTable("purchase_orders");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Aggregates.Supplier", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("varchar(255)")
                        .HasColumnName("address");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("category");

                    b.Property<string>("ContactPerson")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("contact_person");

                    b.Property<string>("Email")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("email");

                    b.Property<string>("LastName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("last_name");

                    b.Property<string>("Name")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("varchar(150)")
                        .HasColumnName("name");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("phone");

                    b.Property<string>("Ruc")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("ruc");

                    b.Property<DateTime>("Since")
                        .HasColumnType("date")
                        .HasColumnName("since");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("status");

                    b.HasKey("Id")
                        .HasName("pk_suppliers");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_suppliers_business_id");

                    b.ToTable("suppliers");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Entities.PurchaseOrderDetail", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<string>("BatchLabel")
                        .HasMaxLength(60)
                        .HasColumnType("varchar(60)")
                        .HasColumnName("batch_label");

                    b.Property<string>("DeliveryStatus")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("varchar(20)")
                        .HasColumnName("delivery_status");

                    b.Property<string>("DeliveryTrackingNum")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("varchar(50)")
                        .HasColumnName("delivery_tracking_num");

                    b.Property<decimal>("Discount")
                        .HasColumnType("decimal(5,4)")
                        .HasColumnName("discount");

                    b.Property<int>("ProductId")
                        .HasColumnType("int")
                        .HasColumnName("product_id");

                    b.Property<int>("PurchaseId")
                        .HasColumnType("int")
                        .HasColumnName("purchase_id");

                    b.Property<int>("Quantity")
                        .HasColumnType("int")
                        .HasColumnName("quantity");

                    b.Property<decimal>("UnitPrice")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("unit_price");

                    b.HasKey("Id")
                        .HasName("pk_purchase_order_details");

                    b.HasIndex("ProductId")
                        .HasDatabaseName("ix_purchase_order_details_product_id");

                    b.HasIndex("PurchaseId")
                        .HasDatabaseName("ix_purchase_order_details_purchase_id");

                    b.ToTable("purchase_order_details");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Entities.SupplierInstallmentPayment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("amount");

                    b.Property<bool>("IsReversed")
                        .HasColumnType("tinyint(1)")
                        .HasColumnName("is_reversed");

                    b.Property<DateTimeOffset>("PaidAt")
                        .HasColumnType("datetime")
                        .HasColumnName("paid_at");

                    b.Property<int>("PaidByUserId")
                        .HasColumnType("int")
                        .HasColumnName("paid_by_user_id");

                    b.Property<DateTimeOffset?>("ReversedAt")
                        .HasColumnType("datetime")
                        .HasColumnName("reversed_at");

                    b.Property<int?>("ReversedByUserId")
                        .HasColumnType("int")
                        .HasColumnName("reversed_by_user_id");

                    b.Property<int?>("SupplierPaymentInstallmentId")
                        .HasColumnType("int")
                        .HasColumnName("supplier_payment_installment_id");

                    b.Property<int>("SupplierPaymentPlanId")
                        .HasColumnType("int")
                        .HasColumnName("supplier_payment_plan_id");

                    b.HasKey("Id")
                        .HasName("pk_supplier_installment_payments");

                    b.HasIndex("SupplierPaymentInstallmentId")
                        .HasDatabaseName("ix_supplier_installment_payments_supplier_payment_installment_id");

                    b.HasIndex("SupplierPaymentPlanId")
                        .HasDatabaseName("ix_supplier_installment_payments_supplier_payment_plan_id");

                    b.ToTable("supplier_installment_payments");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Entities.SupplierPaymentInstallment", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(10,2)")
                        .HasColumnName("amount");

                    b.Property<DateTime>("DueDate")
                        .HasColumnType("date")
                        .HasColumnName("due_date");

                    b.Property<bool>("IsPaid")
                        .HasColumnType("tinyint(1)")
                        .HasColumnName("is_paid");

                    b.Property<int>("Number")
                        .HasColumnType("int")
                        .HasColumnName("number");

                    b.Property<int>("SupplierPaymentPlanId")
                        .HasColumnType("int")
                        .HasColumnName("supplier_payment_plan_id");

                    b.HasKey("Id")
                        .HasName("pk_supplier_payment_installments");

                    b.HasIndex("SupplierPaymentPlanId")
                        .HasDatabaseName("ix_supplier_payment_installments_supplier_payment_plan_id");

                    b.ToTable("supplier_payment_installments");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Entities.SupplierPaymentPlan", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasColumnName("id");

                    b.Property<int>("BusinessId")
                        .HasColumnType("int")
                        .HasColumnName("business_id");

                    b.Property<bool>("IsCancelled")
                        .HasColumnType("tinyint(1)")
                        .HasColumnName("is_cancelled");

                    b.Property<int>("PaidInstallments")
                        .HasColumnType("int")
                        .HasColumnName("paid_installments");

                    b.Property<int>("PurchaseOrderId")
                        .HasColumnType("int")
                        .HasColumnName("purchase_order_id");

                    b.Property<int>("TotalInstallments")
                        .HasColumnType("int")
                        .HasColumnName("total_installments");

                    b.Property<int>("Version")
                        .IsConcurrencyToken()
                        .HasColumnType("int")
                        .HasColumnName("version");

                    b.HasKey("Id")
                        .HasName("pk_supplier_payment_plans");

                    b.HasIndex("BusinessId")
                        .HasDatabaseName("ix_supplier_payment_plans_business_id");

                    b.HasIndex("PurchaseOrderId")
                        .IsUnique()
                        .HasDatabaseName("ix_supplier_payment_plans_purchase_order_id");

                    b.ToTable("supplier_payment_plans");
                });

            modelBuilder.Entity("Kipu.Platform.Alerts.Domain.Model.Aggregates.Alert", b =>
                {
                    b.HasOne("Kipu.Platform.Products.Domain.Model.Entities.Batch", null)
                        .WithMany()
                        .HasForeignKey("BatchId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_alerts_batch_batch_id");

                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_alerts_business_business_id");

                    b.HasOne("Kipu.Platform.Products.Domain.Model.Aggregates.Product", null)
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .HasConstraintName("fk_alerts_product_product_id");

                    b.HasOne("Kipu.Platform.Suppliers.Domain.Model.Aggregates.PurchaseOrder", null)
                        .WithMany()
                        .HasForeignKey("PurchaseOrderId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_alerts_purchase_order_purchase_order_id");

                    b.HasOne("Kipu.Platform.Sales.Domain.Model.Aggregates.Sale", null)
                        .WithMany()
                        .HasForeignKey("SaleId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_alerts_sale_sale_id");

                    b.HasOne("Kipu.Platform.Products.Domain.Model.Aggregates.Warehouse", null)
                        .WithMany()
                        .HasForeignKey("WarehouseId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_alerts_warehouse_warehouse_id");
                });

            modelBuilder.Entity("Kipu.Platform.Alerts.Domain.Model.Entities.AlertRule", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_alert_rules_business_business_id");
                });

            modelBuilder.Entity("Kipu.Platform.Dashboard.Domain.Model.Entities.Report", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_reports_business_business_id");
                });

            modelBuilder.Entity("Kipu.Platform.Iam.Domain.Model.Aggregates.User", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_users_businesses_business_id");

                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Entities.Role", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_users_role_role_id");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Aggregates.Category", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_categories_businesses_business_id");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Aggregates.Product", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_products_businesses_business_id");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Aggregates.Warehouse", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_warehouses_businesses_business_id");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Entities.Batch", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_batches_businesses_business_id");

                    b.HasOne("Kipu.Platform.Products.Domain.Model.Entities.InventoryItem", "InventoryItem")
                        .WithMany()
                        .HasForeignKey("InventoryItemId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_batches_inventory_item_inventory_item_id");

                    b.HasOne("Kipu.Platform.Products.Domain.Model.Aggregates.Product", null)
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_batches_products_product_id");

                    b.Navigation("InventoryItem");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Entities.InventoryItem", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_inventory_items_businesses_business_id");

                    b.HasOne("Kipu.Platform.Products.Domain.Model.Aggregates.Product", null)
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_inventory_items_products_product_id");

                    b.HasOne("Kipu.Platform.Products.Domain.Model.Aggregates.Warehouse", null)
                        .WithMany()
                        .HasForeignKey("WarehouseId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_inventory_items_warehouses_warehouse_id");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Entities.ProductSupplier", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_product_suppliers_businesses_business_id");

                    b.HasOne("Kipu.Platform.Products.Domain.Model.Aggregates.Product", null)
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_product_suppliers_products_product_id");
                });

            modelBuilder.Entity("Kipu.Platform.Products.Domain.Model.Entities.StockMovement", b =>
                {
                    b.HasOne("Kipu.Platform.Products.Domain.Model.Entities.Batch", "Batch")
                        .WithMany()
                        .HasForeignKey("BatchId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_stock_movements_batches_batch_id");

                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_stock_movements_businesses_business_id");

                    b.HasOne("Kipu.Platform.Products.Domain.Model.Aggregates.Product", null)
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_stock_movements_products_product_id");

                    b.HasOne("Kipu.Platform.Products.Domain.Model.Aggregates.Warehouse", null)
                        .WithMany()
                        .HasForeignKey("WarehouseId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_stock_movements_warehouses_warehouse_id");

                    b.Navigation("Batch");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Aggregates.Customer", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_customers_businesses_business_id");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Aggregates.Sale", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_sales_businesses_business_id");

                    b.HasOne("Kipu.Platform.Sales.Domain.Model.Aggregates.Customer", null)
                        .WithMany()
                        .HasForeignKey("CustomerId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_sales_customers_customer_id");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Entities.InstallmentPayment", b =>
                {
                    b.HasOne("Kipu.Platform.Sales.Domain.Model.Entities.PaymentInstallment", null)
                        .WithMany()
                        .HasForeignKey("PaymentInstallmentId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_installment_payments_installment_id");

                    b.HasOne("Kipu.Platform.Sales.Domain.Model.Entities.PaymentPlan", null)
                        .WithMany("Payments")
                        .HasForeignKey("PaymentPlanId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_installment_payments_payment_plan_payment_plan_id");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Entities.PaymentInstallment", b =>
                {
                    b.HasOne("Kipu.Platform.Sales.Domain.Model.Entities.PaymentPlan", null)
                        .WithMany("Installments")
                        .HasForeignKey("PaymentPlanId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_payment_installments_payment_plan_payment_plan_id");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Entities.PaymentPlan", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_payment_plans_businesses_business_id");

                    b.HasOne("Kipu.Platform.Sales.Domain.Model.Aggregates.Sale", null)
                        .WithOne()
                        .HasForeignKey("Kipu.Platform.Sales.Domain.Model.Entities.PaymentPlan", "SaleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_payment_plans_sales_sale_id");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Entities.SaleDetail", b =>
                {
                    b.HasOne("Kipu.Platform.Products.Domain.Model.Aggregates.Product", null)
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_sale_details_products_product_id");

                    b.HasOne("Kipu.Platform.Sales.Domain.Model.Aggregates.Sale", null)
                        .WithMany("SaleDetails")
                        .HasForeignKey("SaleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_sale_details_sales_sale_id");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Aggregates.PurchaseOrder", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_purchase_orders_businesses_business_id");

                    b.HasOne("Kipu.Platform.Suppliers.Domain.Model.Aggregates.Supplier", null)
                        .WithMany()
                        .HasForeignKey("SupplierId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_purchase_orders_supplier_supplier_id");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Aggregates.Supplier", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_suppliers_businesses_business_id");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Entities.PurchaseOrderDetail", b =>
                {
                    b.HasOne("Kipu.Platform.Products.Domain.Model.Aggregates.Product", null)
                        .WithMany()
                        .HasForeignKey("ProductId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_purchase_order_details_products_product_id");

                    b.HasOne("Kipu.Platform.Suppliers.Domain.Model.Aggregates.PurchaseOrder", null)
                        .WithMany("Details")
                        .HasForeignKey("PurchaseId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_purchase_order_details_purchase_orders_purchase_id");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Entities.SupplierInstallmentPayment", b =>
                {
                    b.HasOne("Kipu.Platform.Suppliers.Domain.Model.Entities.SupplierPaymentInstallment", null)
                        .WithMany()
                        .HasForeignKey("SupplierPaymentInstallmentId")
                        .OnDelete(DeleteBehavior.SetNull)
                        .HasConstraintName("fk_supplier_installment_payments_installment_id");

                    b.HasOne("Kipu.Platform.Suppliers.Domain.Model.Entities.SupplierPaymentPlan", null)
                        .WithMany("Payments")
                        .HasForeignKey("SupplierPaymentPlanId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_supplier_installment_payments_plan_id");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Entities.SupplierPaymentInstallment", b =>
                {
                    b.HasOne("Kipu.Platform.Suppliers.Domain.Model.Entities.SupplierPaymentPlan", null)
                        .WithMany("Installments")
                        .HasForeignKey("SupplierPaymentPlanId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_supplier_payment_installments_plan_id");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Entities.SupplierPaymentPlan", b =>
                {
                    b.HasOne("Kipu.Platform.Iam.Domain.Model.Aggregates.Business", null)
                        .WithMany()
                        .HasForeignKey("BusinessId")
                        .OnDelete(DeleteBehavior.Restrict)
                        .IsRequired()
                        .HasConstraintName("fk_supplier_payment_plans_businesses_business_id");

                    b.HasOne("Kipu.Platform.Suppliers.Domain.Model.Aggregates.PurchaseOrder", null)
                        .WithOne()
                        .HasForeignKey("Kipu.Platform.Suppliers.Domain.Model.Entities.SupplierPaymentPlan", "PurchaseOrderId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("fk_supplier_payment_plans_purchase_orders_purchase_order_id");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Aggregates.Sale", b =>
                {
                    b.Navigation("SaleDetails");
                });

            modelBuilder.Entity("Kipu.Platform.Sales.Domain.Model.Entities.PaymentPlan", b =>
                {
                    b.Navigation("Installments");

                    b.Navigation("Payments");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Aggregates.PurchaseOrder", b =>
                {
                    b.Navigation("Details");
                });

            modelBuilder.Entity("Kipu.Platform.Suppliers.Domain.Model.Entities.SupplierPaymentPlan", b =>
                {
                    b.Navigation("Installments");

                    b.Navigation("Payments");
                });
#pragma warning restore 612, 618
        }
    }
}
